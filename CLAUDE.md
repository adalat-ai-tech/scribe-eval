...
4. **Batch Processing** (`src/scribe/measure_batch.py`)
   - `evaluate_records(records, ref_field=..., hyp_field=..., domain_config=None, normalize=True, use_sandhi=True, collect_error_details=False, ...)`: Core in-memory batch API — evaluates any iterable of dicts (parsed JSONL, DataFrame records, training-loop predictions) without touching the filesystem; input dicts are never mutated
