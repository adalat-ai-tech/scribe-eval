...
def frequent_errors_bar_chart(
    freq_data: dict[str, list],
    error_type: str,
    top_n: int = 10,
    output_path: Optional[str] = None,
) -> Optional[object]:
    """
    Generate a horizontal bar chart of the most frequent errors.
    ...
    if error_type == "substitution":
        labels = [f"{ref} -> {hyp}" for ref, hyp, _ in items]
        counts = [count for _, _, count in items]
    else:
        labels = [token for token, _ in items]
    ...
    type_colors = {
        "substitution": COLOR_SUBSTITUTION,
        "deletion": COLOR_DELETION,
        "insertion": COLOR_INSERTION,
    }

    fig, ax = plt.subplots(figsize=(10, max(3, len(labels) * 0.5)))
    ax.barh(range(len(labels)), counts, color=type_colors.get(error_type, "#888888"))
    ax.set_yticks(range(len(labels)))
    ax.set_yticklabels(labels)
    ax.set_xlabel("Count")
    ax.set_title(
        f"Top {len(items)} Frequent {error_type.title()}s",
        fontsize=14,
        fontweight="bold",
    )
