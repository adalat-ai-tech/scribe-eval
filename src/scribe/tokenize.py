...
def domain_aware_tokenizer(
    text: str, domain_config: Optional[DomainConfig] = None
    ...
    if domain_config:
        ...
        domain_with_boundaries = (
            f"(?<!{word_char_pattern})(?:{domain_config.pattern_regex})(?!{word_char_pattern})"
        )
    ...
    punct_split_pattern = (
        rf"(?<!{word_char_pattern})({punct_class})|({punct_class})(?!{word_char_pattern})"
    )
...
if __name__ == "__main__":
    ...
    legal = DomainConfig.legal()
    sample = (
        "U/S 302 of IPC on 22.05.2023 at 10:30, for Rs. 10,500. "
        "ഈ 3-ഓ 4-ഓ വയസ്സുള്ള കുട്ടിക്ക് 9 ാം തീയതി , "
        "9:30-ന് ഫോൺ കിട്ടിയോ? 1,500 rupees abc123def. 19-രംദു & 19-രംദു"
    )
